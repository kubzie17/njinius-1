...
id: get
title: 'Reading'
description: 'How to fetch records from your database with njinius'
---
...
```js {3}
njinius.from(tableName).select(columnQuery)
```
...
```js {3}
njinius.from(tableName).order(columnName, sortAscending, nullsFirst)
```
...
```js {3}
njinius.from(tableName).limit(criteria, foreignTableName)
```
...
```js {3}
njinius.from(tableName).offset(criteria, foreignTableName)
```
...
```js {3}
njinius.from(tableName).range(fromIndex, toIndex)
```
